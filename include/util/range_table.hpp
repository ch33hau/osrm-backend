...
template <unsigned OSRM_BLOCK_SIZE = 16, bool USE_SHARED_MEMORY = false> class RangeTable;

template <unsigned OSRM_BLOCK_SIZE, bool USE_SHARED_MEMORY>
std::ostream &operator<<(std::ostream &out,
                         const RangeTable<OSRM_BLOCK_SIZE, USE_SHARED_MEMORY> &table);

template <unsigned OSRM_BLOCK_SIZE, bool USE_SHARED_MEMORY>
std::istream &operator>>(std::istream &in, RangeTable<OSRM_BLOCK_SIZE, USE_SHARED_MEMORY> &table);
 ...
 *
 * Note: OSRM_BLOCK_SIZE is the number of differential encodoed values.
 * But each block consists of an absolute value and OSRM_BLOCK_SIZE differential values.
 * So the effective block size is sizeof(unsigned) + OSRM_BLOCK_SIZE.
 */
template <unsigned OSRM_BLOCK_SIZE, bool USE_SHARED_MEMORY> class RangeTable
{
  public:
    using BlockT = std::array<unsigned char, OSRM_BLOCK_SIZE>;
    ...
    template <typename VectorT> explicit RangeTable(const VectorT &lengths)
    {
        const unsigned number_of_blocks = [&lengths]() {
            unsigned num = (lengths.size() + 1) / (OSRM_BLOCK_SIZE + 1);
            if ((lengths.size() + 1) % (OSRM_BLOCK_SIZE + 1) != 0)
            {
                num += 1;
            }
            return num;
        ...
        for (const unsigned l : lengths)
        {
            ...
            block_idx = (block_idx + 1) % (OSRM_BLOCK_SIZE + 1);
        }
    ...
    {
        BOOST_ASSERT(id < block_offsets.size() + diff_blocks.size() * OSRM_BLOCK_SIZE);
        // internal_idx 0 is implicitly stored in block_offsets[block_idx]
        const unsigned internal_idx = id % (OSRM_BLOCK_SIZE + 1);
        const unsigned block_idx = id / (OSRM_BLOCK_SIZE + 1);
        ...
        if (internal_idx < OSRM_BLOCK_SIZE)
        {
...
template <unsigned OSRM_BLOCK_SIZE, bool USE_SHARED_MEMORY>
unsigned RangeTable<OSRM_BLOCK_SIZE, USE_SHARED_MEMORY>::PrefixSumAtIndex(int index,
                         ...
                         const RangeTable<OSRM_BLOCK_SIZE, USE_SHARED_MEMORY> &table)
{
    ...
    // write blocks
    out.write((char *)table.diff_blocks.data(), OSRM_BLOCK_SIZE * table.diff_blocks.size());
...
std::istream &operator>>(std::istream &in, RangeTable<OSRM_BLOCK_SIZE, USE_SHARED_MEMORY> &table)
{
    ...
    // read blocks
    in.read((char *)table.diff_blocks.data(), OSRM_BLOCK_SIZE * number_of_blocks);
    return in;
}
} // namespace util
} // namespace osrm
