...
function way_function (way, result)
  ...
  local psv_backward = way:get_value_by_key("psv:backward")
  local psv_backward = way:get_value_by_key("taxi:backward")
  local psv = way:get_value_by_key("psv")
  local route = way:get_value_by_key("route")
  local bridge = way:get_value_by_key("bridge")
  local access_forward = way:get_value_by_key("access:forward")
  local junction = way:get_value_by_key("junction")
    -- local barrier = way:get_value_by_key("barrier", "")
    -- local cycleway = way:get_value_by_key("cycleway", "")
  local service = way:get_value_by_key("service")

  if not ((highway and highway ~= "" and highway ~="pedestrian") or 
      (route and route ~= "") or 
      (bridge and bridge ~= "") or 
      (railway and railway ~= "") or 
      (psv and psv ~= "") or 
      (psv == "yes" and railway == "tram")) then
    return
  end
  ...
  if "" == highway and "" == railway then
    return
  end

  if psv == "yes" and railway == "tram" then
    access = "psv"
  end
  ...
  if obey_oneway then
    ...
    elseif (busway == "opposite_lane" or
    bus_lanes_backward == "designated" or
    busway_left == "opposite_lane" or
    busway_right == "opposite_lane" or
    psv_backward == "yes" or
    taxi_backward == "yes" or
    railway == "tram") then
      result.backward_mode = mode.driving
