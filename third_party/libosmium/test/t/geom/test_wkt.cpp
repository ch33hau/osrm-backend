...
SECTION("point") {
    osrm_osmium::geom::WKTFactory<> factory;
...
SECTION("linestring") {
    ...
    {
        std::string wkt {factory.create_linestring(wnl, osrm_osmium::geom::use_nodes::unique, osrm_osmium::geom::direction::backward)};
        REQUIRE(std::string{"LINESTRING(3.6 4.9,3.5 4.7,3.2 4.2)"} == wkt);
    }

    {
        std::string wkt {factory.create_linestring(wnl, osrm_osmium::geom::use_nodes::all)};
        REQUIRE(std::string{"LINESTRING(3.2 4.2,3.5 4.7,3.5 4.7,3.6 4.9)"} == wkt);
    }

    {
        std::string wkt {factory.create_linestring(wnl, osrm_osmium::geom::use_nodes::all, osrm_osmium::geom::direction::backward)};
