...
TEST_CASE("delta encode int") {
    ...
    osrm_osmium::util::DeltaEncode<int> x;
...
TEST_CASE("delta decode int") {
    ...
    osrm_osmium::util::DeltaDecode<int> x;
...
TEST_CASE("delta encode unsigned int") {
    ...
    osrm_osmium::util::DeltaEncode<unsigned int> x;
...
TEST_CASE("delta decode unsigned int") {
    ...
    osrm_osmium::util::DeltaDecode<unsigned int> x;
...
TEST_CASE("delta encode and decode") {
    ...
    osrm_osmium::util::DeltaEncode<int, int> de;
    std::vector<int> b;
    for (int x : a) {
        b.push_back(de.update(x));
    }

    osrm_osmium::util::DeltaDecode<int, int> dd;
    std::vector<int> c;
    for (int x : b) {
...
TEST_CASE("delta encode iterator") {
    ...
    typedef osrm_osmium::util::DeltaEncodeIterator<std::vector<int>::const_iterator, decltype(l), int> it_type;
