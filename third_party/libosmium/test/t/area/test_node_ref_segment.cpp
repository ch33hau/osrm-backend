...
using osrm_osmium::area::detail::NodeRefSegment;
...
TEST_CASE("NodeRefSegmentClass") {
    ...
    SECTION("instantiation_with_default_parameters") {
        ...
        REQUIRE(s.first().location() == osrm_osmium::Location());
        REQUIRE(s.second().ref() == 0);
        REQUIRE(s.second().location() == osrm_osmium::Location());
    }

    SECTION("instantiation") {
        osrm_osmium::NodeRef nr1(1, { 1.2, 3.4 });
        osrm_osmium::NodeRef nr2(2, { 1.4, 3.1 });
        osrm_osmium::NodeRef nr3(3, { 1.2, 3.6 });
        osrm_osmium::NodeRef nr4(4, { 1.2, 3.7 });
    ...
    SECTION("intersection") {
        ...
        REQUIRE(calculate_intersection(s1, s2) == osrm_osmium::Location(1.0, 1.0));
        REQUIRE(calculate_intersection(s1, s3) == osrm_osmium::Location());
        REQUIRE(calculate_intersection(s2, s3) == osrm_osmium::Location());
        REQUIRE(calculate_intersection(s1, s4) == osrm_osmium::Location());
        REQUIRE(calculate_intersection(s1, s5) == osrm_osmium::Location(2.0, 2.0));
        REQUIRE(calculate_intersection(s1, s1) == osrm_osmium::Location());
        REQUIRE(calculate_intersection(s1, s6) == osrm_osmium::Location());
        REQUIRE(calculate_intersection(s1, s7) == osrm_osmium::Location());
    }
