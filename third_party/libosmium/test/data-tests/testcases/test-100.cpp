...
class TestHandler100 : public osrm_osmium::handler::Handler {
...
public:
    ...
    void node(osrm_osmium::Node& node) {
        if (node.id() == 100000) {
            ...
            REQUIRE(node.timestamp() == osrm_osmium::Timestamp("2014-01-01T00:00:00Z"));
            REQUIRE(node.uid() == 1);
...
TEST_CASE("100") {
    ...
    SECTION("test 100") {
        osrm_osmium::io::Reader reader(dirname + "/1/100/data.osm");
